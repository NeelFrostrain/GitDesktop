...
pub async fn get_gitlab_activity(
    ...
    for proj in project_paths {
        ...
        let repo_name = proj.split('/').next_back().unwrap_or(&proj).to_string();

        // 1. Fetch recent events
        let events_url = format!(
            "{}/api/v4/projects/{}/events?per_page=10",
            clean_url, encoded_proj
        );
        ...
        let pipelines_url = format!(
            "{}/api/v4/projects/{}/pipelines?per_page=5",
            clean_url, encoded_proj
        );
