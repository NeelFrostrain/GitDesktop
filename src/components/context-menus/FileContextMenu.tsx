 ...
 */
export const FileContextMenu: React.FC<FileContextMenuProps> = ({ filePath, x, y, onClose }) => {
  ...
  const applyIgnorePattern = async (pattern: string, logLabel: string) => {
    ...
    try {
      ...
      useLogStore
        .getState()
        .addLog('success', 'Git', `Added '${pattern}' to .gitignore (${logLabel})`);
  ...
  const handleCopyRelativePath = () => {
    ...
    useLogStore
      .getState()
      .addLog('info', 'System', `Copied relative path '${filePath}' to clipboard`);
    onClose();
  };
  ...
  const handleShowInExplorer = async () => {
    try {
      ...
      useLogStore
        .getState()
        .addLog('info', 'System', `Opened file manager at '${fullDir || fullPath}'`);
    } catch (error: unknown) {
      useLogStore
        .getState()
  ...
  const handleOpenDefault = async () => {
    try {
      ...
      useLogStore
        .getState()
        .addLog('info', 'System', `Opened '${filePath}' in default application`);
    } catch (error: unknown) {
      useLogStore
        .getState()
        .addLog('error', 'System', `Failed to open file: ${getErrorMessage(error)}`);
    }
    onClose();
  };
