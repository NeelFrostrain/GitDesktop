  ...
  return (
    <div className="h-full flex flex-col justify-between p-3.5 bg-surface-subtle/80 border border-border/70 rounded-sm select-none font-sans relative">
      ...
      <div className="relative w-full h-[190px] flex items-center justify-center my-auto">
        <svg
          viewBox="0 0 270 190"
          className="w-full h-full overflow-visible max-w-[270px]"
        >
          ...
          >
            ...
            <circle cx={topPoint.x} cy={topPoint.y} r="3.5" fill="#ffffff" stroke="#26a641" strokeWidth="1.5" className="pointer-events-none" />
          </g>
          <g
            ...
            className="cursor-pointer"
          >
            <circle cx={rightPoint.x} cy={rightPoint.y} r="10" fill="transparent" />
            <circle cx={rightPoint.x} cy={rightPoint.y} r="3.5" fill="#ffffff" stroke="#26a641" strokeWidth="1.5" className="pointer-events-none" />
          </g>
          <g
            ...
            className="cursor-pointer"
          >
            <circle cx={bottomPoint.x} cy={bottomPoint.y} r="10" fill="transparent" />
            <circle cx={bottomPoint.x} cy={bottomPoint.y} r="3.5" fill="#ffffff" stroke="#26a641" strokeWidth="1.5" className="pointer-events-none" />
          </g>
          <g
            ...
            className="cursor-pointer"
          >
            <circle cx={leftPoint.x} cy={leftPoint.y} r="10" fill="transparent" />
            <circle cx={leftPoint.x} cy={leftPoint.y} r="3.5" fill="#ffffff" stroke="#26a641" strokeWidth="1.5" className="pointer-events-none" />
          </g>
        </svg>
        ...
        >
          ...
          <p className="text-[10px] text-text-muted font-medium">Pull requests</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-1.5 pt-2 border-t border-border/50 text-[10.5px]">
        <div
          className={`flex items-center gap-1.5 px-2 py-1 bg-base-0/80 border rounded-xs transition-colors cursor-pointer ${
            hoveredAxis === 'Commits' ? 'border-commito-coral/50 bg-commito-coral/5' : 'border-border'
          }`}
          onMouseEnter={() => setHoveredAxis('Commits')}
          onMouseLeave={() => setHoveredAxis(null)}
        >
          ...
          <span className="font-semibold text-text-primary ml-auto">{breakdown.commits_pct}%</span>
        </div>
        <div
          className={`flex items-center gap-1.5 px-2 py-1 bg-base-0/80 border rounded-xs transition-colors cursor-pointer ${
            hoveredAxis === 'Pull requests' ? 'border-purple-400/50 bg-purple-500/5' : 'border-border'
          }`}
          onMouseEnter={() => setHoveredAxis('Pull requests')}
          onMouseLeave={() => setHoveredAxis(null)}
        >
          ...
          <span className="font-semibold text-text-primary ml-auto">{breakdown.prs_pct}%</span>
        </div>
        <div
          className={`flex items-center gap-1.5 px-2 py-1 bg-base-0/80 border rounded-xs transition-colors cursor-pointer ${
            hoveredAxis === 'Issues' ? 'border-emerald-400/50 bg-emerald-500/5' : 'border-border'
          }`}
          onMouseEnter={() => setHoveredAxis('Issues')}
          onMouseLeave={() => setHoveredAxis(null)}
        >
          ...
          <span className="font-semibold text-text-primary ml-auto">{breakdown.issues_pct}%</span>
        </div>
        <div
          className={`flex items-center gap-1.5 px-2 py-1 bg-base-0/80 border rounded-xs transition-colors cursor-pointer ${
            hoveredAxis === 'Code review' ? 'border-blue-400/50 bg-blue-500/5' : 'border-border'
          }`}
          onMouseEnter={() => setHoveredAxis('Code review')}
          onMouseLeave={() => setHoveredAxis(null)}
        >
          ...
          <span className="font-semibold text-text-primary ml-auto">{breakdown.reviews_pct}%</span>
        </div>
      </div>
