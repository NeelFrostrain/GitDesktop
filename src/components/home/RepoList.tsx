...
import { Search, DownloadCloud, FolderOpen, Plus, FolderGit2 } from 'lucide-react';
...
/**
 * Dashboard repository grid with search and quick-launch actions.
 */
      ...
      {filteredRepos.length > 0 ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '10px' }}>
          {filteredRepos.map((repo) => (
            <RepoCard key={repo.id} repo={repo} status={statuses[repo.path]} />
          ))}

          {/* Add tile */}
          <div
            onClick={handleOpenFolderDialog}
            className="border border-dashed border-border hover:border-border-strong bg-transparent hover:bg-base-1 rounded-sm p-5 flex flex-col items-center justify-center text-center cursor-pointer transition group min-h-[120px]"
          >
            <div className="w-7 h-7 rounded-sm bg-base-1 border border-border flex items-center justify-center text-text-faint group-hover:text-text-muted group-hover:border-border-strong transition mb-2">
              <Plus className="w-4 h-4" />
            </div>
            <span className="text-[11px] font-medium text-text-muted group-hover:text-text-primary transition">
              Add Repository
            </span>
