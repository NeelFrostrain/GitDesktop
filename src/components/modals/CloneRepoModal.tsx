...
export const CloneRepoModal: React.FC = () => {
  ...
  // Auth State
  const [authMode, setAuthMode] = useState<AuthMode>(() => (accounts.length > 0 ? 'saved' : 'credentials'));
  ...
  const handleClone = async (e: React.FormEvent) => {
    ...
    try {
      ...
      useLogStore
        .getState()
        .addLog('success', 'Git', `Successfully cloned repository '${repoName}' to '${fullDestinationPath}'`);
      ...
      if (msg.toLowerCase().includes('authentication failed') || msg.toLowerCase().includes('fatal: authentication')) {
        formattedError = 'Authentication failed. Please check your username and password or Personal Access Token (PAT). If 2FA is active, use a PAT.';
      } else if (msg.toLowerCase().includes('already exists') || msg.toLowerCase().includes('not empty')) {
            ...
            <div className="space-y-3">
              ...
              {authMode === 'saved' && accounts.length > 0 && (
                <div className="space-y-1.5 pt-1">
                  <label className="text-xs font-medium text-text-secondary">Connected Account</label>
                  <Dropdown
                      ...
                      <div className="flex items-start gap-2 text-xs text-text-primary font-medium">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-text-muted leading-tight mt-0.5">
                            When 2FA is active, Git requires a <strong>Personal Access Token (PAT)</strong> with repository read permissions, or an active TOTP verification code.
                          </p>
                        </div>
                      </div>
              ...
              {authMode === 'public' && (
                <div className="p-2.5 bg-base-1/70 border border-border/60 rounded-sm flex items-center gap-2 text-xs text-text-secondary">
                  ...
                  <span>No authentication credentials required for public open-source repositories.</span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                <label className="text-xs font-medium text-text-secondary flex items-center gap-1.5">
                  ...
                  <span>Branch / Tag <span className="text-text-muted font-normal text-[11px]">(optional)</span></span>
                </label>
                <input
                  type="text"
