...
export const DiffViewer: React.FC = () => {
  ...
  const renderChangesDiff = () => {
    ...
    const additionsCount = diff.lines.filter((l) => l.line_type === 'addition').length;
    const deletionsCount = diff.lines.filter((l) => l.line_type === 'deletion').length;

    return (
      <div className="h-full flex flex-col">
        <div className="h-10 bg-base-1 border-b border-border px-3.5 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-2 truncate min-w-0">
            ...
            {(additionsCount > 0 || deletionsCount > 0) && (
              <div className="flex items-center gap-1.5 font-mono text-[11px] font-semibold ml-1">
                {additionsCount > 0 && <span className="text-emerald-400">+{additionsCount}</span>}
                {deletionsCount > 0 && <span className="text-red-400">-{deletionsCount}</span>}
              </div>
            )}
            <CopyButton text={selectedFile} />
          </div>
    ...
    return (
      <div className="h-full flex flex-col overflow-hidden">
        ...
        <div className="p-3 bg-base-2 border-b border-border space-y-2.5 flex-shrink-0">
          ...
          {/* Author, Commit Node, and Additions/Deletions Bar (Matching Image 1) */}
          <div className="flex items-center justify-between text-[11px] bg-base-1 px-2.5 py-1.5 rounded-md border border-border/80 shadow-sm">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="flex items-center gap-1 text-text-muted font-mono text-[10px] px-1.5 py-0.2 rounded bg-base-2 border border-border/60">
                ...
                <CopyButton text={commitDetails.commit.sha} className="p-0.5 border-0 bg-transparent hover:bg-base-3 text-[9px]" />
              </div>
          ...
          <div className="space-y-2">
            {commitDetails.changed_files.map((file) => {
              ...
              return (
                ...
                >
                  {/* File Accordion Header (Matching Image 1) */}
                  <button
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      {/* Per-file additions / deletions if present */}
                      {fileStat && (fileStat.additions > 0 || fileStat.deletions > 0) && (
                        <div className="flex items-center gap-1.5 text-[11px] font-mono font-semibold">
                          {fileStat.additions > 0 && (
                            <span className="text-emerald-400">+{fileStat.additions}</span>
                          )}
                          {fileStat.deletions > 0 && (
                            <span className="text-red-400">-{fileStat.deletions}</span>
                          )}
                        </div>
                      )}
